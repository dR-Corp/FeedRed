...
class SSP {
	...
	static function filter ( $request, $columns, &$bindings )
	{
		...
		if ( $where !== '' ) {
			$join = (isset($request['join']) && $request['join']) ? 'redaction.idArticle = article.idArticle AND ' : '';
	...
	static function bind ( &$a, $val, $type )
	{
		...
		if(count($a) == 2) {
			...
			$sql = "SELECT idRedacteur FROM redacteur, users WHERE redacteur.idUser = users.idUser AND CONCAT(nom,' ',prenom) LIKE '%$val%'";
			$res = $bdd->query($sql)->fetch( PDO::FETCH_ASSOC );
			$val = $res ? $res['idRedacteur'] : $val;
		}
